...
JobsIn 是一个面向求职流程的 AI 工作台：帮你采集岗位、按简历匹配度排序、生成沟通文案，并通过 Chrome 扩展完成投递执行。

> AI-powered job search, resume-based matching, and application workflow in one place.

## 为什么值得用

- 🔍 **更快发现岗位**：集中采集岗位信息，减少重复搜索与手动翻页
- 🤖 **AI 推荐排序**：根据简历与 JD 做匹配分析，优先看到更值得投递的岗位
- ✍️ **生成可编辑文案**：基于岗位内容生成沟通文案，生成、编辑、复制一体完成
- 📊 **跟踪投递流程**：从采集、分析、推荐到投递状态，统一放在一个界面里管理
- 🚀 **真实浏览器执行**：通过 Chrome 扩展配合系统 Chrome 执行流程，保留登录态与实际浏览器环境

## 适合谁

- 想提升 job search 效率、减少重复操作的求职者
- 希望用 AI job recommendations 快速筛岗位的人
- 需要把简历匹配、岗位管理、application workflow 放到一个工具里的人

## 核心能力

- 多平台岗位采集：支持 Boss直聘（已实现）、智联招聘 / 前程无忧 / 猎聘（架构已预留）
- AI 多维度岗位匹配评分（技能 / 经验 / 学历 / 薪资）
- AI 根据岗位 JD + 简历生成个性化沟通文案
- Web 界面：岗位浏览、AI 推荐排序、采集任务管理、投递状态追踪
- 一键投递 / 批量投递：通过 Chrome Extension 自动化执行
